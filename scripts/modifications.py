...
# %% Measuring similarity between quotes for administrative purposes
import numpy
import difflib
...
def filter_by_similarity(difference_matrix, tolerance=0.4):
    ...
    with numpy.nditer(difference_matrix, flags=["multi_index"]) as it:
        for similarity_measure in it:
            if (similarity_measure >= tolerance) and (len(set(it.multi_index)) != 1):
                simular_quotes = numpy.append(
                    simular_quotes,
...
simular_quotes_list = filter_by_similarity(difference,
                                           tolerance=0.5)  # not really a list but the name still suits it...
